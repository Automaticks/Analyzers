...
{
    /// <summary>Diagnostic rule emitted when a folder exceeds <see cref="MaxFiles" /> direct source files.</summary>
        ...
        ConcurrentDictionary<string, ConcurrentDictionary<string, Location>> filesByFolder)
    {
        ...
        var location = Location.Create(treeContext.Tree, new TextSpan(0, 0));
        var files = filesByFolder.GetOrAdd(directory, _ => new ConcurrentDictionary<string, Location>());
        ...
        ConcurrentDictionary<string, ConcurrentDictionary<string, Location>> filesByFolder)
    {
        foreach (var folderEntry in filesByFolder)
        {
            ...
            foreach (var fileEntry in folderEntry.Value)
            {
