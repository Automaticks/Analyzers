...
{
    /// <summary>Fires when a property or indexer is immediately adjacent to another member with no blank line between them.</summary>
    ...
    private static void AnalyzeTypeDeclaration(SyntaxNodeAnalysisContext context)
    {
        ...
        for (var index = 0; index < members.Count - 1; index++)
        {
            ...
            if (!IsPropertyOrIndexer(first) && !IsPropertyOrIndexer(second))
            {
                continue;
            }
    ...
    private static bool IsPropertyOrIndexer(MemberDeclarationSyntax member)
    {
        return member is PropertyDeclarationSyntax or IndexerDeclarationSyntax;
    }

    private static bool HasBlankLine(MemberDeclarationSyntax first, MemberDeclarationSyntax second)
    {
