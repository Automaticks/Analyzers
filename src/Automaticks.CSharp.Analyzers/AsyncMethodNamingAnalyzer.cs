...
using static Automaticks.CSharp.AsyncReturnTypeHelper;

namespace Automaticks.CSharp;

/// <summary>
///     Flags methods that return a task type or <c>IAsyncEnumerable&lt;T&gt;</c>
///     but whose name does not end with the <c>Async</c> suffix.
/// </summary>
[DiagnosticAnalyzer(LanguageNames.CSharp)]
public sealed class AsyncMethodNamingAnalyzer : DiagnosticAnalyzer
{
    /// <summary>
    ///     The diagnostic rule reported when an async-returning method name is missing the
    ///     <c>Async</c> suffix.
    /// </summary>
    ...
    private static void AnalyzeMethod(SyntaxNodeAnalysisContext context)
    {
        ...
        if (!ReturnsAsyncType(symbol, context.SemanticModel.Compilation))
        {
            return;
        }

        if (symbol.IsOverride && IsExternalOverride(symbol))
        {
            return;
        }
