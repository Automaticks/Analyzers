...
/// <summary>
///     Tests that RefParameterAnalyzer exempts members bound to interfaces that come from metadata.
/// </summary>
public class RefParameterAnalyzerExternalInterfaceTests
{
    /// <summary>
    ...
    public async Task Analyze_ExplicitMetadataInterfaceImplementation_ReportsNoDiagnostic(CancellationToken cancellationToken)
    {
        ...
        await Assert.That(DiagnosticCollectionAssertions.HasId(diagnostics, DiagnosticIds.CSharp.RefParameterForbidden)).IsFalse();
    }
    ...
    /// <summary>
    ///     Tests that Analyze_LocalFunctionWithNonRefModifier_ReportsNoDiagnostic.
    /// </summary>
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>A task representing the asynchronous test operation.</returns>
    [Test]
    public async Task Analyze_LocalFunctionWithNonRefModifier_ReportsNoDiagnostic(CancellationToken cancellationToken)
    {
        const string source = """
                              namespace MyApp {
                                  public class Foo {
